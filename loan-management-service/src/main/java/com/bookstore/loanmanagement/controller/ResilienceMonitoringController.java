...
public class ResilienceMonitoringController {
        ...
        Map<String, Object> circuitBreakers = circuitBreakerRegistry.getAllCircuitBreakers()
            ...
            .collect(Collectors.toMap(
                ...
                cb -> {
                    ...
                    details.put("numberOfSlowCalls", cb.getMetrics().getNumberOfSlowCalls());
                    return details;
                }
            ));
    ...
    public ResponseEntity<Map<String, Object>> getCircuitBreaker(@PathVariable String name) {
        try {
            ...
            details.put("numberOfSuccessfulCalls", cb.getMetrics().getNumberOfSuccessfulCalls());
            details.put("numberOfSlowCalls", cb.getMetrics().getNumberOfSlowCalls());
            details.put("numberOfNotPermittedCalls", cb.getMetrics().getNumberOfNotPermittedCalls());
    ...
    @PostMapping("/circuit-breakers/{name}/transition-to-closed")
    public ResponseEntity<Map<String, String>> transitionToClosed(@PathVariable String name) {
        try {
            CircuitBreaker cb = circuitBreakerRegistry.circuitBreaker(name);
            cb.transitionToClosedState();
            log.info("Circuit breaker {} transitioned to CLOSED state", name);
            return ResponseEntity.ok(Map.of("message", "Circuit breaker transitioned to CLOSED", "name", name));
        } catch (Exception e) {
            log.error("Failed to transition circuit breaker to CLOSED: {}", name, e);
            return ResponseEntity.badRequest().body(Map.of("error", "Failed to transition circuit breaker"));
        }
    }
